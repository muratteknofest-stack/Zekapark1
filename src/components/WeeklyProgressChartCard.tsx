...
export const WeeklyProgressChartCard: React.FC<WeeklyProgressChartCardProps> = ({
  ...
  return (
    <div
      id="progress-panel"
      className={`bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm transition-all ${className}`}
    >
      {/* Header Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        {/* View Switcher Tabs */}
        <div className="flex items-center p-1 rounded-2xl bg-slate-100 border border-slate-200 self-start md:self-auto shrink-0">
          <button
            onClick={() => {
              sound.playClick();
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 my-5">
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-50 to-indigo-100/50 border border-indigo-200/80">
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        {onStartPractice && (
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-sm shadow-indigo-600/20 active:scale-98 transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
          >
